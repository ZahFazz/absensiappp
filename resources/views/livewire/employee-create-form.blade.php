<div>
    <form wire:submit.prevent="saveEmployees" method="post" novalidate>
        ...
        <div class="mb-3">
            <div class="w-100">
                <div class="mb-3">
                    <x-form-label id="name{{ $i }}" label='Teacher Name {{ $i + 1 }}' />
                ...
                <div class="mb-3">
                    <x-form-label id="phone{{ $i }}" label='Telephone Number {{ $i + 1 }}' />
                ...
                <div class="mb-3">
                    <x-form-label id="password{{ $i }}"
                        label='Teacher Password {{ $i + 1 }} (default: "123")' required="false" />
                ...
                <div class="mb-3">
                    <x-form-label id="position_id{{ $i }}" label='Subject {{ $i + 1 }}' />
            ...
            <button class="btn btn-sm btn-danger mt-2" wire:click="removeEmployeeInput({{ $i }})"
                wire:target="removeEmployeeInput({{ $i }})" type="button" wire:loading.attr="disabled">Delete</button>
            @endif
        </div>
        <hr>
        @endforeach
